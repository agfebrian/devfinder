<template>
  <layouts-card
    class="flex items-center gap-6 px-[10px] py-[9.5px] shadow-app shadow-[#4660bb33] dark:shadow-none"
  >
    <div class="flex flex-1 items-center pl-[22px]">
      ...
      <p
        v-if="status == 'error'"
        class="pl-2 text-[15px] font-bold text-app-red"
      >
        No Results
      </p>
    </div>
    <button
      @click="onClick"
      class="flex h-[50px] w-[106px] items-center justify-center rounded-[10px] bg-app-primary text-base font-bold text-white transition hover:bg-app-primary-hover active:bg-app-primary-hover"
    >
      Search
    </button>
  </layouts-card>
...
onMounted(() => inputSearch.value!.focus());
</script>
