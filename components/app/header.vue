<template>
  <div class="flex items-center justify-between">
    <h2 class="text-[26px] font-bold text-app-grey-5 dark:text-white">
      devfinder
    </h2>
    <button
      @click="toggle"
      class="flex items-center gap-4 bg-transparent outline-none"
    >
      <p v-if="showLabel == 'dark'" class="text-[13px] font-bold uppercase">
        {{ showLabel }}
      </p>
      <p v-else class="text-[13px] font-bold uppercase text-white">
        {{ showLabel }}
      </p>
      <svg
        width="20"
...
watch(theme, (newVal) => (colorMode.preference = newVal));
